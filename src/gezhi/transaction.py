...
from .migration import LegacyMigrationRequired, guard_legacy_repository_lock
    ...
    *,
    ...
    try:
        with _repository_lock(canonical_root):
            return _commit_transaction_locked(canonical_root, journal_path, fault=fault)
    except LegacyMigrationRequired as exc:
        return TransactionResult(False, True, str(exc), journal_path)
...
def repository_lock_path(canonical_root: Path) -> Path:
    ...
    if dot_git.is_dir():
        return dot_git / "gezhi-repository.lock"
    digest = hashlib.sha256(str(canonical_root).encode("utf-8")).hexdigest()
    return Path.home() / ".cache" / "gezhi" / "repository-locks" / f"{digest}.lock"
...
def _apply_step(canonical_root: Path, transaction_dir: Path, step: dict[str, Any]) -> None:
    ...
    temp_path = destination.parent / f".{destination.name}.gezhi-{uuid.uuid4().hex}.tmp"
...
def _repository_lock(canonical_root: Path) -> Iterator[None]:
    ...
    with lock_path.open("a+b") as lock_file:
        ...
        try:
            with guard_legacy_repository_lock(canonical_root):
                yield
        finally:
