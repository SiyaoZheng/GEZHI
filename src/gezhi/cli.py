...
from .migration import LegacyMigrationRequired, ensure_init_target_is_migrated
from .runtime import (
    DEFAULT_MAX_MINUTES,
    RuntimeOptions,
    cleanup_runtime,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="gezhi",
        description="Configure and run GEZHI artifact-centered heartbeats for coding agents.",
        epilog="Omitting the command defaults to run. Use 'gezhi <command> -h' for subcommand options.",
    )
    parser.add_argument("-c", "--config", default="gezhi.toml", help="Path to gezhi.toml (default: gezhi.toml)")
    subparsers = parser.add_subparsers(dest="command", title="commands")

    subparsers.add_parser("init", help="Create a starter artifact gezhi.toml")
    subparsers.add_parser("validate", help="Validate gezhi.toml, prompt placeholders, and writable scopes")
    ...
    heartbeat_install.add_argument("--force", action="store_true", help="Overwrite an existing GEZHI-managed service file")
    ...
    cleanup_parser.add_argument("--kill-orphans", action="store_true", help="Terminate orphan GEZHI/Codex processes for this project when no live heartbeat lock exists")
...
def init_config(config_path: Path) -> int:
    ...
    try:
        ensure_init_target_is_migrated(config_path)
