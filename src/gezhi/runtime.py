...
from .migration import (
    ...
    command_argument_tokens,
    ensure_runtime_is_migrated,
    is_claude_print_command,
    is_codex_exec_command,
    is_gezhi_runtime_command,
    is_legacy_runtime_command,
)
...
class HeartbeatRecorder:
    ...
    def heartbeat(self, phase: str, run_dir: Path | None) -> None:
        ...
        self.telemetry.add_event("gezhi.heartbeat", attributes)
        self.telemetry.pulse("gezhi.heartbeat", attributes)
...
def run_goal(config: GoalConfig, options: RuntimeOptions | None = None, adapters: GoalProviderAdapters | None = None) -> RunResult:
    ...
    migration_result = _migration_preflight(config)
...
class HeartbeatRunner:
    ...
    def run(self) -> RunResult:
        with HeartbeatLock(self.config.lock_path, self.config.safety.lock_stale_seconds):
            ...
            if self.state.get("operator_stopped") is True:
                ...
                return RunResult(0, WorkState.STOPPED, None, "perpetual goal is stopped; run gezhi resume to continue")
    ...
    def _run_tik_providers(self, providers: tuple[TikConfig, ...], artifact: dict[str, Any], run_dir: Path) -> tuple[TikProviderReview, ...]:
        ...
        with ThreadPoolExecutor(max_workers=len(providers), thread_name_prefix="gezhi-tik") as executor:
            futures = {
...
def _migration_preflight(config: GoalConfig) -> RunResult | None:
    try:
        ensure_runtime_is_migrated(config.root, config.state_dir, config.runs_dir, config.name)
    except LegacyMigrationRequired as exc:
        return RunResult(2, "migration_required", None, str(exc))
    return None
...
def _terminate_orphan_goal_processes(config: GoalConfig) -> list[str]:
    ...
    for line in result.stdout.splitlines():
        ...
        if pid == current_pid:
            continue
        if (
            not is_codex_exec_command(command)
            and not is_claude_print_command(command)
            and not is_gezhi_runtime_command(command)
            and not is_legacy_runtime_command(command)
        ):
            continue
        if not _command_references_project(command, config.root):
            continue
        try:
    ...
    if not actions:
        actions.append("no orphan GEZHI/Codex processes found for this project")
    return actions


def _command_references_project(command: str, root: Path) -> bool:
    arguments = command_argument_tokens(command)
    explicit_paths = _project_option_paths(arguments)
    candidates = explicit_paths or arguments
...
def _project_option_paths(arguments: tuple[str, ...]) -> tuple[str, ...]:
    value_options = {"-c", "--config", "-C", "--cd", "--cwd"}
    equals_options = ("--config=", "--cd=", "--cwd=")
    values: list[str] = []
    for index, argument in enumerate(arguments):
        if argument in value_options and index + 1 < len(arguments):
            values.append(arguments[index + 1])
            continue
        for prefix in equals_options:
