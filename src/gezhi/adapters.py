...
def run_tik(
    ...
    if config.provider in COMMAND_TIK_PROVIDERS:
        env = {
            "GEZHI_ARTIFACT": str(artifact_path),
            "GEZHI_TIK_PROMPT": prompt,
            "GEZHI_RUN_DIR": str(run_dir),
        }
        ok = run_shell_logged(
            config.command or "",
            root,
...
def build_api_tik_prompt(config: TikConfig, root: Path, prompt: str) -> str:
    ...
    return (
        "Apply this local GEZHI tik skill while reviewing the attached artifact.\n\n"
        f"<gezhi-tik-skill name=\"{config.skill}\">\n"
        f"Source: {skill_path}\n\n"
        f"{skill_text}\n"
        "</gezhi-tik-skill>\n\n"
        "<gezhi-tik-task>\n"
