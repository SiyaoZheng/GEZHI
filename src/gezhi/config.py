...
from .migration import LegacyMigrationRequired, ensure_config_identity_is_migrated, ensure_legacy_state_is_migrated
...
API_TIK_ENV_FILE_ENV_VAR = "GEZHI_API_ENV_FILE"
...
class NoMistakesConfig:
    ...
    mode: str = "lightspeed"
    branch_prefix: str = "gezhi"
    intent: str | None = None
    skip_steps: tuple[str, ...] = ()
    timeout_seconds: float = 0.0
    checkpoint_message: str = "GEZHI checkpoint: {goal_name} heartbeat {iteration} {phase}"


@dataclass(frozen=True)
class ObservabilityConfig:
    enabled: bool = True
    service_name: str = "gezhi"
...
def load_config(config_path: str | Path = "gezhi.toml") -> GoalConfig:
    path = Path(config_path)
    if not path.is_absolute():
        path = Path.cwd() / path
    try:
        ensure_config_identity_is_migrated(path)
    ...
    state_dir = _path(raw.get("state_dir", ".gezhi"), root, root)
    runs_dir = _path(raw.get("runs_dir", ".gezhi/runs"), root, root)
    try:
    ...
    no_mistakes = NoMistakesConfig(
        ...
        branch_prefix=str(no_mistakes_raw.get("branch_prefix", "gezhi")).strip(),
        ...
        checkpoint_message=str(
            no_mistakes_raw.get(
                "checkpoint_message",
                "GEZHI checkpoint: {goal_name} heartbeat {iteration} {phase}",
            )
        ).strip(),
    )
    ...
    observability = ObservabilityConfig(
        ...
        service_name=str(observability_raw.get("service_name", "gezhi")).strip(),
...
def api_tik_env_file_paths() -> tuple[Path, ...]:
    ...
    return (config_home / "gezhi" / "api.env",)
