...
def configure_observability(config: GoalConfig) -> GoalTelemetry:
    ...
    if not _INITIALIZED:
        ...
        _PROVIDER = provider
        _TRACER = trace.get_tracer("gezhi")
        _INITIALIZED = True
        atexit.register(_shutdown_provider)
    return GoalTelemetry(True, plan.reason, _TRACER or trace.get_tracer("gezhi"), plan.kind, plan.path)
...
def _package_version() -> str:
    try:
        return importlib.metadata.version("gezhi")
