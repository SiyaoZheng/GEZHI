---
name: gezhi-project-setup
description: Use when connecting an existing project to gezhi for a non-expert user. Find the finished thing, create a reliable rebuild command, write gezhi.toml, validate the setup, and run the first safe checks.
version: 1.0.0
---
...
under `gezhi`, especially when the user can judge the finished thing but does
...
The deliverable is not just a `gezhi.toml`. The deliverable is a repeatable
thing loop:
...
- If a command fails, preserve the exact failing command and the relevant output
  ...
  pass.
- Do not edit unrelated README/docs/tests/source while setting up gezhi unless
  that file is directly required for the producer or goal config.

## Workflow

### 0. Verify gezhi and Required Tools
...
If `gezhi` is missing, verify Python 3.11+ and install from GitHub:
...
If you are inside a checked-out `gezhi` repository, prefer the local checkout:
...
The producer is the command `gezhi` will run on every heartbeat. It must
...
Before writing `gezhi.toml`, run the producer once if it is safe. If it fails,
...
`PACKYCODE_CODEX_KEY`, `OPENAI_API_KEY`, or `~/.config/gezhi/api.env`.
...
Start from `gezhi init` when no config exists, or edit the existing
`gezhi.toml` carefully.
...
fixed timer; gezhi defaults to a 5-minute wake-up for perpetual goals and a
...
- `gezhi validate` fails: fix the config shape, placeholders, write scopes,
  or forbidden runtime prompt language before any other check.
- `gezhi doctor` fails on a missing command: install the missing tool only
...
- `gezhi.toml` points at the producer and artifact.
...
- `gezhi validate` passes.
- `gezhi doctor` reports the configured static setup as ready or the
...
- The final response to the user names the artifact, producer command,
  ...
  is appropriate, the next command should be `gezhi heartbeat install ...`;
  otherwise it should be one manual `gezhi run ...`.
