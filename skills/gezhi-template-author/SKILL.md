---
name: gezhi-template-author
description: Use when adding or maintaining reusable gezhi templates, artifact recipes, tik oracle scripts, example gezhi.toml files, or project-type setup guidance.
version: 1.0.0
---
...
- One-prompt setup material must be usable by an agent that has never seen the
  target project and by a user who cannot debug `gezhi.toml` by hand.
...
gezhi verdict. The script should check the artifact and print a JSON object
with:
...
`OPENAI_API_KEY`, or `~/.config/gezhi/api.env`. Keep `store = false` unless
...
that `gezhi doctor` checks the command statically.
...
Omit `--every-minutes` unless the template needs a fixed timer; gezhi
...
```text
skills/gezhi-project-setup/SKILL.md
skills/gezhi-template-author/SKILL.md
docs/skills.md
examples/<project-family>/gezhi.toml
...
- one-prompt setup docs explain how to install or verify `gezhi`;
